...
def mean_gamma_deviance(y_true, y_pred, multioutput="uniform_average"):
    if multioutput == "uniform_average":
        ...
        return np.array(
            [_mean_gamma_deviance(yt, yp) for yt, yp in zip(y_true.T, y_pred.T)]
        )
...
def mean_poisson_deviance(y_true, y_pred, multioutput="uniform_average"):
    if multioutput == "uniform_average":
        ...
        return np.array(
            [_mean_poisson_deviance(yt, yp) for yt, yp in zip(y_true.T, y_pred.T)]
        )
...
test_list = [
    (pm.MeanSquaredError, mean_squared_error, None),
...
def test_single_update_gpu_batch(metric, baseline, constrain):
    ...
    m = metric()
    m_val = m(torch.tensor(pred, device="cuda"), torch.tensor(target, device="cuda"))
    base_val = baseline(pred, target, multioutput="raw_values")
